      ...
      <v-col v-for="card in cards" :key="card.title" :cols="3">
        <v-card class="card-cv">
          ...
          <v-card-actions>
            ...
            <v-btn
              color="#00b14f"
              style="color: white; min-width: unset"
              @click="editCV(card.id)"
            >
...
export default {
  ...
  methods: {
    ...
    previewCV(resumeId) {
      ...
      var namePath = this.cards.filter((item) => item.id == resumeId)[0][
        "path"
      ];
      //   var basePath = "D:/PhuocTam/file/";
      var basePath = "http://127.0.0.1:8887/";
      window.open(basePath + namePath, "_blank", "fullscreen=yes");
    },
